   ...
   if(buttonDisable === false){
    ...
    const findIndex=()=>{
        holdSeqall.forEach((val,idx)=>{
            ...
            if (holdvar.length === val.length && holdvar.every((v, i) => v === val[i])) {
                holdidx=idx
              }
        ...
        if( Math.max(...vardviewSeq) < carosel.length){
          ...
          a3.classList.add("c1")
            ...
            setTimeout(() => {
             ...
             if(Math.max(...vardviewSeq) < carosel.length){
                const  hold = document.getElementById(`m${Math.max(...vardviewSeq)+1}`)
                hold.classList.remove('removecard')
                hold.classList.remove('box-move-main-right')
                hold.classList.remove('box-move-left-to-main')
                hold.classList.remove('check')
    ...
    return(
        <div className="Crouselbox">
            ...
            <p className="techheadText">Certifications</p>
                ...
                <CaroselView imageNumber={1} text={"Completed Meta's Programming with Js course on Coursera."}  classN={"c1"}   classN2={"m1"}/>
                <CaroselView imageNumber={2} text={"Completed Meta's Intro to Mobile App Development course on Coursera."} classN={"c2"} classN2={"m2"}/>
                <CaroselView imageNumber={3} text={"Completed Meta'sversion Controlling course on Coursera."} classN={"c3"}  classN2={"m3"} />
                <CaroselView imageNumber={4} text={"participated in Teknofest an International Event."} classN2={"m4"}  classN={"removecard"}/>
                <CaroselView imageNumber={5} text={"ational Event."} classN2={"m5"}  classN={"removecard"}/>
                <CaroselView imageNumber={6} text={"Event."} classN2={"m6"}  classN={"removecard"}/>
